//! When reading from `AtomicSdarc` it firstly loads pointer then increment the reference count.
//!
...
impl ReaderCriticalSection {
    ...
    pub fn reader_critical_section<R>(&self, func: impl FnOnce() -> R) -> R {
        ...
        /// Release ordering: it should synchronize-with the Acquire ordering load in
        /// [`Self::spin_until_observing_non_critical_section_once_in_each_shard`]
        /// Acquire ordering: ensure that the loading of atomic pointer is after
