...
fn set_env(shard_count: usize, collector_interval_ms: u64) {
    unsafe {
        ...
        std::env::set_var(
            "RUST_SDARC_COLLECTOR_INTERVAL_MS",
            collector_interval_ms.to_string(),
        );
    }
}
...
fn scenario_integrated() {
    ...
    {  // ---- Build the hot pool ----
        ...
        let producers: Vec<thread::JoinHandle<()>> = (0..3)
            .map(|p| {
                                ...
                                if spawned.len() < 12 {
                                    ...
                                    let child = thread::spawn(move || {
                                        ...
                                        let _ = child_senders[rng2.usize(child_senders.len())].send(
                                            Sdarc::new(CacheEntry::new(rng2.next(), rng2.next(), 999)),
                                        );
            ...
            rt.block_on(async {
                ...
                let bg_handle = tokio::spawn(async move {
                    ...
                    while !bg_ctx.stop.load(Ordering::Relaxed) {
                        ...
                        tokio::time::sleep(Duration::from_millis(25 + rng.usize(55) as u64)).await;
                    }
                });
        ...
        assert!(
            context.invariant_ok.load(Ordering::Relaxed),
            "invariant violated"
        );
        ...
        eprintln!(
            "  after channel drain: tracked={}, slots={}",
            tracked_alloc_count(),
        ...
        eprintln!(
            "  after context drop: tracked={}, slots={}",
            tracked_alloc_count(),
            crate::sharded_alloc::total_sharded_alloc_used_slots()
        );
    }
    ...
    async move {
        ...
        while !ctx.stop.load(Ordering::Relaxed) {
            match rng.usize(8) {
                ...
                2 => {
                    // Simulate async I/O.
                    tokio::time::sleep(Duration::from_micros(rng.usize(250) as u64)).await;
                }
                3 => {
