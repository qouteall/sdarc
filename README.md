...
This library provides sharded-deferred-atomic-reference-counting (`Sdarc`). It can be used similar to `Arc`. A thread increment/decrement one counter shard (In Linux the shard is determined by `sched_getcpu`. In other platforms it's determined by thread id hash). The `Sdarc` can be freely sent and shared between threads, so one counter shard may become negative. This reduces cache contention of incrementing/decrementing counter. 
...
It has a background thread periodically do collection (the background thread will be launched after first usage of `Sdarc`). This is similar to GC. But it's not tracing GC. It doesn't traverse the object graph.
...
- If you want it to drop content immediately when strong reference count goes 0. (Sdarc collector frees layer-by-layer by default. A deep structure may take long time to be fully freed.)
...
- Linux percpu-refcount has a special owner and has two stages. In the first stage, increment/decrement use the per-CPU ref count slot. When the original owner drops reference, it switches to one atomic counter using RCU. In `Sdarc` there is no special owner, similar to `Arc`.
